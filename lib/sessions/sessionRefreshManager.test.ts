...
describe('SessionRefreshManager', () => {
  ...
  describe('Refresh checks', () => {
    ...
    it('should allow refresh after a previous refresh errored', async () => {
      refreshManager.start();
      mockSessionManager._sessionTimeRemaining = 3 * 60 * 60 * 1000;
      ...
      // Second call should proceed (isRefreshing was reset in finally block).
      mockSessionManager.refreshSession.mockResolvedValueOnce(true);
      await refreshManager.forceRefreshCheck();

      expect(mockSessionManager.refreshSession).toHaveBeenCalledTimes(2);
    });
    ...
    it('should stop when refresh returns false (terminal decline)', async () => {
      refreshManager.start();
      mockSessionManager._sessionTimeRemaining = 3 * 60 * 60 * 1000;
  ...
  describe('Integration tests', () => {
    ...
    it('should properly re-register listeners after stop and restart', async () => {
      mockSessionManager._sessionTimeRemaining = 3 * 60 * 60 * 1000;
      ...
      // Clear mocks so we can verify the second lifecycle independently.
      vi.clearAllMocks();
      mockSessionManager.refreshSession.mockResolvedValue(true);
      mockSessionManager._sessionTimeRemaining = 3 * 60 * 60 * 1000;

      // Second lifecycle — should re-register all listeners and work again.
      refreshManager.start();
      expect(refreshManager.isActive()).toBe(true);
