...
export class OriginKeyManager {
    ...
    async getOrCreateOriginKey(): Promise<OriginKeyData> {
        try {
            ...
            throw new Error(
                `Failed to get or create origin key: ${
    ...
    async saveOriginKey(
        ...
        try {
            ...
            const data = {
                ...
                originKey: originKey, // Store non-extractable CryptoKey directly
                expiresAt,
                createdAt: Date.now()
            };
    ...
    async clearOriginKey(): Promise<void> {
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                request.onsuccess = () => resolve();
                request.onerror = () =>
                    reject(new Error('Failed to clear origin key'));
            });
        } catch (error) {
            console.warn('Failed to clear origin key:', error);
        }
    }
