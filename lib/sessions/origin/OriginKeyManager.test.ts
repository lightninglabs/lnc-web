...
import { OriginKeyManager, OriginKeyData } from './OriginKeyManager';
...
const createFakeIndexedDB = (): FakeIndexedDB => {
    ...
    class FakeDatabase {
        ...
        transaction(storeNamesArg: string[], mode: IDBTransactionMode) {
...
describe('OriginKeyManager', () => {
    ...
    describe('getOrCreateOriginKey()', () => {
        it('should return existing key if not expired', async () => {
            ...
            vi.spyOn(originKeyManager as any, 'loadOriginKey').mockResolvedValue(existingKeyData);
        ...
        it('should create new key if no existing key found', async () => {
            ...
            vi.spyOn(originKeyManager as any, 'loadOriginKey').mockResolvedValue(undefined);
            // Mock saveOriginKey to succeed
            vi.spyOn(originKeyManager as any, 'saveOriginKey').mockResolvedValue(undefined);
        ...
        it('should create new key if existing key is expired', async () => {
            const expiredKeyData: OriginKeyData = {
                originKey: { type: 'expired' } as CryptoKey,
                expiresAt: Date.now() - 3600000 // 1 hour ago
            };

            // Mock loadOriginKey to return expired key
            vi.spyOn(originKeyManager as any, 'loadOriginKey').mockResolvedValue(expiredKeyData);
            // Mock saveOriginKey to succeed
            vi.spyOn(originKeyManager as any, 'saveOriginKey').mockResolvedValue(undefined);
        ...
        it('should handle key generation failure', async () => {
            ...
            globalThis.crypto.subtle.generateKey.mockRejectedValue(error);
        ...
        it('should handle save failure', async () => {
            ...
            vi.spyOn(originKeyManager as any, 'saveOriginKey').mockRejectedValue(error);
    ...
    describe('loadOriginKey()', () => {
        it('should be tested via integration with getOrCreateOriginKey', async () => {
            ...
            vi.spyOn(originKeyManager as any, 'loadOriginKey').mockResolvedValue(keyData);
