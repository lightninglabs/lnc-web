...
export class SessionStorage {
  ...
  load(): SessionData | undefined {
    ...
    // Return cached result if the cache has been populated. The cache uses
    // null for "loaded but empty" while the public API returns undefined, so
    // the nullish coalescing converts between the two representations.
    ...
    try {
      ...
      if (!stored) {
        // null = "loaded but empty" (distinct from undefined = "not yet loaded").
        this._cache = null;
        return undefined;
      }
