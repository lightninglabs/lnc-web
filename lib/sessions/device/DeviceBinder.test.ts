...
describe('DeviceBinder', () => {
  ...
  beforeEach(() => {
    ...
    mockCanvas.toDataURL.mockReturnValue(
      'data:image/png;base64,test-canvas-data'
    );
  ...
  describe('deriveSessionKey()', () => {
    ...
    it('should derive session key from fingerprint and session ID', async () => {
      const result = await deviceBinder.deriveSessionKey(
        fingerprint,
        sessionId
      );
    ...
    it('should use correct info parameter', async () => {
      ...
      const expectedInfoBytes = new TextEncoder().encode(
        'lnc-session-device-key'
      );
      expect(mockDeriveKey).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('should handle importKey failure', async () => {
      ...
      await expect(
        deviceBinder.deriveSessionKey(fingerprint, sessionId)
      ).rejects.toThrow('Import key failed');
    });
    ...
    it('should handle deriveKey failure', async () => {
      ...
      await expect(
        deviceBinder.deriveSessionKey(fingerprint, sessionId)
      ).rejects.toThrow('Derive key failed');
    });
  });
  ...
  describe('Private methods', () => {
    describe('generateCanvasFingerprint()', () => {
      ...
      it('should handle canvas context unavailable', async () => {
        ...
        await expect(
          (deviceBinder as any).generateCanvasFingerprint()
        ).rejects.toThrow(
          'Canvas fingerprinting required for session security'
        );
      });
    ...
    describe('hashFingerprint()', () => {
      ...
      it('should handle digest failure', async () => {
        ...
        await expect(
          (deviceBinder as any).hashFingerprint('test')
  ...
  describe('Integration tests', () => {
    it('should generate consistent fingerprints for same device properties', async () => {
      ...
      mockCanvas.toDataURL.mockReturnValue(
        'data:image/png;base64,consistent-data'
      );
    ...
    it('should handle full fingerprint and key derivation workflow', async () => {
      ...
      const sessionKey = await deviceBinder.deriveSessionKey(
        fingerprint,
        sessionId
      );
