...
describe('SessionManager', () => {
  ...
  beforeEach(() => {
    ...
    // Reset mock defaults
    mockDeviceBinder.generateFingerprint.mockResolvedValue('test-fingerprint');
    ...
    mockKeyWrapper.unwrapCredentialsKey.mockResolvedValue(new Uint8Array(32));
  ...
  describe('Constructor', () => {
    it('should initialize with default config', () => {
      expect(sessionManager.config.sessionDuration).toBe(24 * 60 * 60 * 1000);
    ...
    it('should initialize all services with namespace', () => {
      expect(CredentialsEncrypter).toHaveBeenCalledWith(mockCryptoService);
  ...
  describe('createSession()', () => {
    it('should create session successfully', async () => {
      ...
      expect(mockOriginKeyManager.getOrCreateOriginKey).toHaveBeenCalled();
    ...
    it('should throw error when session creation fails', async () => {
      ...
      ).rejects.toThrow('Fingerprint error');
    });
  ...
  describe('canAutoRestore', () => {
    it('should return true when session storage has data', () => {
      storedSessionData = mockSessionData;
      expect(sessionManager.canAutoRestore).toBe(true);
    });

    it('should return false when session storage has no data', () => {
      storedSessionData = null;
      expect(sessionManager.canAutoRestore).toBe(false);
    });
  });
  ...
  describe('refreshSession()', () => {
    ...
    it('should successfully refresh session', async () => {
      ...
      expect(mockSessionStorage.save).toHaveBeenCalled();
      expect(mockSessionStorage.save.mock.calls[0][0].refreshCount).toBe(1);
      expect(
  ...
  describe('sessionTimeRemaining', () => {
    it('should return 0 when no session data', () => {
      ...
      const result = sessionManager.sessionTimeRemaining;
  ...
  describe('Private methods', () => {
    describe('generateSecureSessionId()', () => {
      it('should use crypto.randomUUID when available', () => {
        ...
        const result = (sessionManager as any).generateSecureSessionId();
  ...
  describe('Integration tests', () => {
    it('should handle full session lifecycle', async () => {
      ...
      // Get time remaining
      const timeRemaining = sessionManager.sessionTimeRemaining;
    ...
    it('should handle session expiry', async () => {
      ...
      // Check expired state
      expect(sessionManager.hasActiveSession).toBe(false);
      expect(sessionManager.sessionTimeRemaining).toBe(0);
