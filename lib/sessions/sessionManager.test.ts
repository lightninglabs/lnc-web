...
describe('SessionManager', () => {
  ...
  it('returns the in-flight promise for concurrent refresh calls without duplicating work', async () => {
    await manager.createSession(baseCredentials);

    // Make the first refreshSession block until we resolve the promise.
    let resolveEncrypt!: (value: EncryptedCredentials) => void;
    vi.mocked(internals.encrypter.encrypt).mockReturnValueOnce(
      new Promise<EncryptedCredentials>((r) => {
        resolveEncrypt = r;
      })
    );

    // Start the first refresh — it will block inside encrypt.
    const first = manager.refreshSession();

    // The second call receives the same in-flight promise so it sees the
    ...
    resolveEncrypt(encryptedCredentials);
    await expect(first).resolves.toBe(true);
    await expect(second).resolves.toBe(true);
  });

  it('propagates in-flight errors to concurrent refresh callers', async () => {
    await manager.createSession(baseCredentials);

    // Make the origin key lookup fail so refreshSessionInternal throws.
    vi.spyOn(internals.originKeyManager, 'loadOriginKey').mockResolvedValue(
      undefined
    );

    // Both callers should see the same infrastructure error rather than
    // the second caller receiving a misleading success.
    const first = manager.refreshSession();
    const second = manager.refreshSession();

    await expect(first).rejects.toThrow('Origin key missing');
    await expect(second).rejects.toThrow('Origin key missing');
  });
  ...
  it('clears storage when restoreSession catches an infrastructure error', async () => {
    ...
    const clearSpy = vi.spyOn(internals.storage, 'clear');
    await manager.restoreSession();

    expect(clearSpy).toHaveBeenCalled();
  });

  it('clears storage when session is expired', async () => {
    await manager.createSession(baseCredentials);
  ...
  it('clears storage when no session data exists', async () => {
    const clearSpy = vi.spyOn(internals.storage, 'clear');
    const result = await manager.restoreSession();
  ...
  describe('refreshSession limits', () => {
    ...
    it('returns false when session age equals maxSessionAgeMs exactly', async () => {
      const shortAge = 2 * 60 * 60 * 1000; // 2 hours
      manager = new SessionManager(namespace, {
        sessionDurationMs: 1 * 60 * 60 * 1000,
        maxSessionAgeMs: shortAge
      });
      internals = manager as unknown as SessionManagerInternals;
      ...
      // Advance to exactly the max session age boundary.
      vi.advanceTimersByTime(shortAge);
