...
export default class SessionRefreshManager {
  ...
  private async checkAndRefreshSession(): Promise<void> {
    try {
      ...
      const activityThresholdMs = this.config.activityThreshold * 60 * 1000;
      ...
      const timeUntilExpiry = this.sessionManager.getSessionTimeRemaining();
      const refreshTriggerMs = this.config.refreshTrigger * 60 * 60 * 1000; // Convert hours to ms
      ...
      if (timeUntilExpiry > 0 && timeUntilExpiry < refreshTriggerMs) {
        ...
        if (refreshed) {
          ...
          log.info('Session refresh failed or reached limits');
        }
      }
    } catch (error) {
      log.error('Error during session refresh check:', error);
    }
  }
