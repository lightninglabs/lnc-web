...
export default class SessionRefreshManager {
  ...
  // Config is snapshotted at construction time. Changes to the session
  // manager's config after construction will not be observed.
  ...
  private async checkAndRefreshSession(): Promise<void> {
    ...
    try {
      ...
      if (this.consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
        // Design note: the circuit breaker intentionally stops silently
        // (no callback / user notification) after repeated infrastructure
        // failures. This is deliberate — the refresh manager is a best-effort
        // background optimization, and surfacing transient crypto/storage
