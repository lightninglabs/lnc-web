...
vi.mock('@lightninglabs/lnc-core', () => ({
  snakeKeysToCamel: vi.fn((value: unknown) => value)
}));
...
describe('WasmManager', () => {
  ...
  describe('request', () => {
    ...
    it('passes parsed response through snakeKeysToCamel', async () => {
      const namespace = 'request-camel';
      const wasm = createWasmNamespace({
        wasmClientInvokeRPC: vi
          .fn()
          .mockImplementation(
      ...
      // Override the identity mock with a realistic transformation.
      vi.mocked(snakeKeysToCamel).mockImplementationOnce((value) => ({
        channelId: (value as Record<string, unknown>).channel_id,
        remoteBalance: (value as Record<string, unknown>).remote_balance
      }));

      const manager = new WasmManager(namespace, 'code', createCallbacks());
      const result = await manager.request('lnrpc.ChannelBalance', {});

      expect(result).toEqual({ channelId: '123', remoteBalance: 50000 });
      expect(snakeKeysToCamel).toHaveBeenCalledWith({
        channel_id: '123',
        remote_balance: 50000
      });
    });
  ...
  describe('subscribe', () => {
    ...
    it('passes parsed response through snakeKeysToCamel before onMessage', () => {
      const namespace = 'subscribe-camel';
      const wasm = createWasmNamespace({
        wasmClientInvokeRPC: vi
          .fn()
          .mockImplementation(
            (_method: string, _req: string, cb: (res: string) => void) => {
              cb(JSON.stringify({ channel_id: '456', chan_point: 'abc:0' }));
            }
          )
      });
      registerNamespace(namespace, wasm);
      ...
      vi.mocked(snakeKeysToCamel).mockImplementationOnce((value) => ({
        channelId: (value as Record<string, unknown>).channel_id,
        chanPoint: (value as Record<string, unknown>).chan_point
      }));
