...
/**
 * Config after merging user options with DEFAULT_CONFIG. Fields covered by
 * DEFAULT_CONFIG are guaranteed present after the merge; session and
 * pairingPhrase remain optional because they have no sensible default.
 * Using Required<LightningNodeConnectConfig> would be unsound — it would
 * claim session is SessionConfig when it is actually undefined at runtime.
 */
type ResolvedConfig = Required<
  Omit<LightningNodeConnectConfig, 'session' | 'pairingPhrase'>
> &
  Pick<LightningNodeConnectConfig, 'session' | 'pairingPhrase'>;
...
export default class LightningNodeConnect {
  ...
  constructor(lncConfig?: LightningNodeConnectConfig) {
    const config = Object.assign(
      {},
      DEFAULT_CONFIG,
      // Default passkeyDisplayName incorporates the resolved namespace.
      { passkeyDisplayName: `LNC User (${lncConfig?.namespace || 'default'})` },
      lncConfig
    ) as ResolvedConfig;
  ...
  get serverHost(): string {
    // Read from the credential cache first, falling back to the resolved config value.
  ...
  set serverHost(host: string) {
    this._credentialCache.set('serverHost', host);
  }
   ...
   */
  async pair(pairingPhrase: string, persist?: PersistOptions): Promise<void> {
    // Ensure the background session init is complete before overwriting cache
    // to prevent a race where tryAutoRestore() could overwrite fresh pairing
    // credentials with stale session data.
    await this._authCoordinator.waitForSessionRestoration();
    ...
    if (persist) {
      if (persist.method === 'password') {
        await this.persistWithPassword(persist.password);
      } else {
        await this.persistWithPasskey(persist);
      }
    }
  }
   ...
   */
  async login(options: UnlockOptions): Promise<void> {
    const success = await this.unlock(options);
    if (!success) {
      throw new Error(`Failed to unlock with method '${options.method}'`);
    }
    await this.connect();
  }
   ...
   */
  async persistWithPasskey(
    options?: Partial<PasskeyUnlockOptions>
  ): Promise<void> {
    if (!this.isConnected) {
      throw new Error('Must be connected before persisting credentials');
    }
