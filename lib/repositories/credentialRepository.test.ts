...
describe('BaseCredentialRepository', () => {
    ...
    describe('hasCredential()', () => {
        ...
        it('should trigger load when credentials map is empty', () => {
            mockLocalStorage.getItem.mockReturnValue(JSON.stringify({ 'stored-key': 'stored-value' }));

            const result = repository.hasCredential('stored-key');

            expect(result).toBe(true);
            expect(mockLocalStorage.getItem).toHaveBeenCalledWith('lnc-web:test-namespace');
        });
    });
    ...
    describe('clear()', () => {
        it('should clear all credentials and save empty state', () => {
            ...
            expect(mockLocalStorage.removeItem).toHaveBeenCalledWith('lnc-web:test-namespace');
        });
    ...
    describe('Private methods', () => {
        ...
        describe('loadedCredentials getter', () => {
            it('should load credentials when map is empty', () => {
                mockLocalStorage.getItem.mockReturnValue(JSON.stringify({ 'loaded-key': 'loaded-value' }));
        ...
        describe('load()', () => {
            it('should load credentials from localStorage', () => {
                const storedData = { 'stored-key1': 'stored-value1', 'stored-key2': 'stored-value2' };
                mockLocalStorage.getItem.mockReturnValue(JSON.stringify(storedData));

                (repository as any).load();

                expect(repository['credentials'].get('stored-key1')).toBe('stored-value1');
                expect(repository['credentials'].get('stored-key2')).toBe('stored-value2');
                expect(console.log).toHaveBeenCalledWith(
    ...
    describe('Integration tests', () => {
        ...
        it('should persist data across instances with same namespace', () => {
            ...
            // Trigger load
            newRepository.hasAnyCredentials();
