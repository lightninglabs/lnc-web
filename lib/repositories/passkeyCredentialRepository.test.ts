...
const mockEncryptionService = {
  ...
  encrypt: vi.fn(),
  get isUnlocked() {
    return mockEncryptionService._isUnlocked;
  },
  _isUnlocked: false,
  lock: vi.fn(),
...
describe('PasskeyCredentialRepository', () => {
  ...
  beforeEach(() => {
    ...
    mockEncryptionService._isUnlocked = false;
  ...
  describe('Constructor', () => {
    it('should create instance with namespace and encryption service', () => {
      const repo = new PasskeyCredentialRepository(
        'test-ns',
        mockEncryptionService as any
      );
  ...
  describe('Integration tests', () => {
    it('should support full unlock, set, get, lock workflow', async () => {
      ...
      // Unlock with passkey
      mockEncryptionService._isUnlocked = true; // Simulate successful unlock
      ...
      // Lock repository
      mockEncryptionService._isUnlocked = false; // Simulate lock
      repository.lock();
      expect(repository.isUnlocked).toBe(false);
    });
    ...
    it('should work with different namespaces', () => {
      const repo1 = new PasskeyCredentialRepository(
        'namespace1',
        mockEncryptionService as any
      );
      const repo2 = new PasskeyCredentialRepository(
        'namespace2',
        mockEncryptionService as any
      );
