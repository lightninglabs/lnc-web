...
describe('AuthenticationCoordinator', () => {
  ...
  beforeEach(() => {
    ...
    // Reset mock defaults
    mockStrategyManager.getStrategy.mockImplementation((method: any) => {
      if (method === 'passkey') {
        return {
          method: 'passkey',
          isSupported: () => false,
  ...
  describe('Constructor', () => {
    it('should create instance with dependencies', () => {
      ...
      expect(authCoordinator['sessionCoordinator']).toBe(
        mockSessionCoordinator
      );
    });
  });
  ...
  describe('unlock()', () => {
    it('should return false when strategy is not supported', async () => {
      ...
      const result = await authCoordinator.unlock({
        method: 'password'
      } as any);
    ...
    it('should load credentials from strategy after unlock', async () => {
      mockStrategy.unlock.mockResolvedValue(true);
      mockStrategy.isUnlocked.mockReturnValue(true);
      mockStrategy.getCredential.mockImplementation((key) =>
        Promise.resolve(`value-${key}`)
      );

      await authCoordinator.unlock({ method: 'password' } as any);
    ...
    it('should persist cached credentials to strategy', async () => {
      mockStrategy.unlock.mockResolvedValue(true);
      mockStrategy.isUnlocked.mockReturnValue(true);
      mockCredentialCache.get.mockImplementation((key: any) => `cached-${key}`);

      await authCoordinator.unlock({ method: 'password' } as any);

      expect(mockStrategy.setCredential).toHaveBeenCalledWith(
        'localKey',
        'cached-localKey'
      );
      expect(mockStrategy.setCredential).toHaveBeenCalledWith(
    ...
    it('should create session when sessions are available and unlocked', async () => {
      mockStrategy.unlock.mockResolvedValue(true);
      mockStrategy.isUnlocked.mockReturnValue(true);
      mockCredentialCache.get.mockImplementation((key: any) => `value-${key}`);
  ...
  describe('tryAutoRestore()', () => {
    ...
    it('should restore session successfully', async () => {
      ...
      expect(authCoordinator['activeStrategy']).toEqual({
        unlock: expect.any(Function),
        method: 'session'
      });
      expect(mockCredentialCache.hydrateFromSession).toHaveBeenCalledWith(
        sessionCredentials
      );
    });
  ...
  describe('clearSession()', () => {
    it('should clear session and reset state', () => {
      ...
      expect(spy).toHaveBeenCalledWith(
        '[AuthenticationCoordinator] Cleared session state'
      );
    });
  });
  ...
  describe('Private methods', () => {
    ...
    describe('waitForSessionRestoration()', () => {
      it('should return immediately when session not available', async () => {
        ...
        await expect(
          (authCoordinator as any).waitForSessionRestoration()
    ...
    describe('persistCachedCredentials()', () => {
      ...
      it('should handle errors when persisting credentials', async () => {
        ...
        mockStrategy.setCredential.mockRejectedValue(
    ...
    describe('loadCredentialsFromStrategy()', () => {
      it('should load credentials from strategy into cache', async () => {
        ...
        await (authCoordinator as any).loadCredentialsFromStrategy(
          mockStrategy
        );
    ...
    describe('saveCredentialToStrategy()', () => {
      it('should save credential to active strategy', async () => {
        ...
        await (authCoordinator as any).saveCredentialToStrategy(
          'test-key',
          'test-value'
        );
  ...
  describe('Integration tests', () => {
    it('should handle full authentication lifecycle', async () => {
      ...
      const unlockResult = await authCoordinator.unlock({
        method: 'password'
      } as any);
